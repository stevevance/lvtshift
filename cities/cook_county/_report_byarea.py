...
Reads data/parcels_modeled.parquet (built by _build_data.py) and renders a single
...
gs = pd.read_parquet('data/parcels_modeled.parquet')
...
<p style="font-size:12.5px;color:#8C8B87;margin:6px 0 4px">Cook County variants: <b>C1 · single county-wide rate</b> &nbsp;·&nbsp; <a href="cook-c2-mv.html" style="color:#5C6FE3;text-decoration:none">C2-MV · per-district (market value)</a> &nbsp;·&nbsp; <a href="cook-c2-cp.html" style="color:#5C6FE3;text-decoration:none">C2-CP · per-district (classification-preserved)</a></p>
<header><h1>Cook County: a single county-wide land value tax (C1)</h1>
<p class='lede'>This scenario replaces the entire Cook County property tax — every taxing body combined,
about $19.2B for tax year 2024 — with one revenue-neutral land tax across the whole county. A parcel's
current bill is its gross equalized value times its tax code's composite rate (Cook County Clerk).</p>
<p class='lede'>Two reforms are shown: a 2.5:1 split-rate (land taxed 2.5× buildings, the Illinois
classification cap) and a 100% building abatement (a pure land tax). Tax changes are solved county-wide;
each view is the slice of parcels in the selected township. Positive means the parcel pays more.</p>
<p class='lede'>One caveat: a single county-wide rate also pools tax base across the whole county — a much
bigger change than a land tax. The per-district variants (C2-MV and C2-CP) keep each district's own
revenue and are the more realistic answer to "what does a land tax do."</p></header>
<div class='controls'>
...
Cityscape. <b>Limitation:</b> current tax uses gross equalized value (homeowner/senior exemptions and
TIF not removed), so Σ current ≈ $20.8B vs the official ~$19.2B extension (+8%); dollar levels run
slightly high, but the revenue-neutral distribution is unaffected. Background:
...
Path('cook_report_bytownship.html').write_text(html)
print('Wrote cook_report_bytownship.html (%d KB)' % (len(html)//1024))
