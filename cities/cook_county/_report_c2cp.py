...
Reads data/parcels_modeled_c2cp.parquet (built by _build_data.py) and renders a single
...
gs = pd.read_parquet('data/parcels_modeled_c2cp.parquet')
...
function render(){
  ...
  document.getElementById('extremes').innerHTML=
    "Across townships under this reform — <b>highest net change:</b> "+hi+" ("+money(DATA.byArea[hi][nk])+") &nbsp;·&nbsp; <b>lowest net change:</b> "+lo+" ("+money(DATA.byArea[lo][nk])+")";
...
Path('cook_report_c2cp_bytownship.html').write_text(html)
print('Wrote cook_report_c2cp_bytownship.html (%d KB)' % (len(html)//1024))
