...
Caches data/parcels_modeled_c2.parquet for the renderer.
"""
import os
import sys
...
codesum = df[tax].groupby('tax_code').agg(eav=('eav','sum'),
            L=('market_land','sum'), B=('market_bldg','sum'))
...
ml, mb = df['market_land'], df['market_bldg']
...
out.to_parquet('data/parcels_modeled_c2.parquet')
print(f'cached {len(out):,} parcels → data/parcels_modeled_c2.parquet', flush=True)
